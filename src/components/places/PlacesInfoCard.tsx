...
export default function PlacesInfoCard({ selectedPref }: { selectedPref: string }) {
  ...
  return (
    ...
    >
      ...
      <Text type="secondary" style={{ fontWeight: 500 }}>代表会场：</Text>
      <div className="flex flex-wrap gap-2 mb-4">
        {info.venues.split('、').map((venue, idx) => (
          <Tag color="pink" key={venue+idx} style={{ marginBottom: 4 }}>{venue}</Tag>
        ))}
      </div>
      <Button className="!w-full" type="primary" href={`/places/prefecture/${getIdByPrefectureName(selectedPref)}`}>探索{selectedPref}的所有会场</Button>
    </Card>
  );
}
